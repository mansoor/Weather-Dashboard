...
# Build a .env from injected environment variables if one isn't already present.
# Laravel's key:generate and package:discover both require a writable .env on disk.
if [ ! -f "$ENV_FILE" ]; then
...
# Generate APP_KEY into the .env file only when it is blank.
