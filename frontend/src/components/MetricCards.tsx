...
export default function MetricCards({ reading }: { reading: WeatherReading }) {
  ...
  const cards = [
    {
      key: 'wind', label: 'Wind', icon: Wind, color: 'text-cyan-500 dark:text-cyan-400', bg: 'bg-cyan-50 dark:bg-cyan-900/20',
      main: `${fmt(reading.wind_speed, 1)} km/h`,
      sub: reading.wind_gusts != null ? `Gusts ${fmt(reading.wind_gusts, 1)} km/h` : 'No gust data',
    },
    {
      key: 'rain', label: 'Precipitation', icon: Umbrella, color: 'text-blue-500 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-900/20',
    ...
    {
      key: 'clouds', label: 'Cloud Cover', icon: Cloud, color: 'text-slate-500 dark:text-slate-400', bg: 'bg-slate-100 dark:bg-slate-700/20',
    ...
    {
      key: 'uv', label: 'UV Index', icon: Sun, color: 'text-yellow-500 dark:text-yellow-400', bg: 'bg-yellow-50 dark:bg-yellow-900/20',
    ...
    {
      key: 'pressure', label: 'Pressure', icon: Gauge, color: 'text-purple-500 dark:text-purple-400', bg: 'bg-purple-50 dark:bg-purple-900/20',
    ...
    {
      key: 'humidity', label: 'Humidity', icon: Zap, color: 'text-teal-500 dark:text-teal-400', bg: 'bg-teal-50 dark:bg-teal-900/20',
        ...
        return (
          <div key={card.key} className={`glass rounded-xl p-4 ${card.bg}`}>
            <div className="flex items-center justify-between mb-3">
              <span className="text-slate-500 dark:text-slate-400 text-xs">{card.label}</span>
