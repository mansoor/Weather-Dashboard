                        ...
                        <div className="flex gap-2">
                          {[6, 24, 48, 168].map(h => (
                            <button key={h} onClick={() => setHistoryHours(h)}
                              className={`px-2 py-1 rounded text-xs transition-colors ${historyHours === h ? 'bg-sky-600 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-400 dark:hover:bg-slate-600'}`}>
                              {h < 48 ? `${h}h` : `${h / 24}d`}
                            </button>
                          ))}
                        </div>
